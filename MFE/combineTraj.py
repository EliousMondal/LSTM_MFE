...
# ρt  = np.zeros((param.NSteps, 2), dtype=np.complex128)
ρt  = np.zeros((param.NSteps, 2))
...
for iTraj in TaskArray:
    ...
    try:
        ...
        ρt[:, 0] += np.abs(ψTraj[:, 0])**2
        ρt[:, 1] += np.abs(ψTraj[:, 1])**2
        count += 1
        os.remove(f"Data/{iTraj+1}/psi_t_{iTraj+1}_λ{param.λ}.txt")
    except:
        continue
...
if comm.rank == 0:
    ...
    np.savetxt(f"Data/rho11_t_λ{param.λ}.txt", ρBf[:, 1])
