...
ψFdum, ψBdum, δεtdum, δεcdum = method.evolve(R_dum, P_dum, 2, 1, 0)
print(f"complied for process {rank}")

μMat_sp = np.array([1.0, 1.0])
μMat_id = np.array([[0, 1], [1, 0]])
...
for iTraj in TaskArray:
    ...
    for iState in range(μMat_sp.shape[0]):
        ...
        np.savetxt(f"Data/{iTraj+1}/energy_t_{iTraj+1}_{iF}{iB}.txt", δεt)
        np.savetxt(f"Data/{iTraj+1}/energy_CPA_t_{iTraj+1}_{iF}{iB}.txt", δεc)
